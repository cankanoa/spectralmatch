        ...
        if image_parallel_workers is not _UNSET:
            if image_parallel_workers is not None:
                if (
                        ...
                        or (image_parallel_workers[1] != "cpu" and not isinstance(image_parallel_workers[1], int))
                ):
                    raise ValueError(
        ...
        if window_parallel_workers is not _UNSET:
            if window_parallel_workers is not None:
                if (
                        ...
                        or (window_parallel_workers[1] != "cpu" and not isinstance(window_parallel_workers[1], int))
                ):
                    raise ValueError(
    ...
    def validate_local_block_adjustment(
        ...
        if number_of_blocks is not _UNSET:
            if not (
                    ...
                    or (isinstance(number_of_blocks, tuple) and len(number_of_blocks) == 2 and all(
                isinstance(i, int) for i in number_of_blocks))
        ...
        if correction_method is not _UNSET:
            if correction_method not in {"gamma", "linear"}:
                raise ValueError("correction_method must be either 'gamma' or 'linear'.")

        if save_block_maps is not _UNSET:
            if save_block_maps is not None:
                if not (isinstance(save_block_maps, tuple) and len(save_block_maps) == 2 and all(
                        isinstance(s, str) for s in save_block_maps)):
                    raise ValueError("save_block_maps must be a tuple of two strings or None.")
        ...
        if load_block_maps is not _UNSET:
            if load_block_maps is not None:
                if not (
                        ...
                        and (
                                (isinstance(load_block_maps[0], str) or load_block_maps[0] is None)
                                and (isinstance(load_block_maps[1], list) or load_block_maps[1] is None)
                        )
                ):
                    raise ValueError("load_block_maps must be a tuple (str|None, list[str]|None) or None.")
        ...
        if override_bounds_canvas_coords is not _UNSET:
            if override_bounds_canvas_coords is not None:
                if not (
                        ...
                        and all(isinstance(v, (float, int)) for v in override_bounds_canvas_coords)
                ):
                    raise ValueError("override_bounds_canvas_coords must be a tuple of four floats or ints, or None.")

        if block_valid_pixel_threshold is not _UNSET:
            if not isinstance(block_valid_pixel_threshold, (float, int)) or not (0 <= block_valid_pixel_threshold <= 1):
